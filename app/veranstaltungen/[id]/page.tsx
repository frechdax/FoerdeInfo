...
function shortDescription(event: {
  ...
  const fallback = `${event.title} am ${formatDate(event.date)} an der Flensburger Förde: Termin, Ort, Veranstalter und Originalquelle.`;
...
export default async function VeranstaltungDetailPage({ params }: PageProps) {
  ...
  const timeLabel = event.time
    ? `${String(event.time).slice(0, 5)} Uhr`
  ...
  return (
    <>
      ...
      <SeoLanding
        eyebrow="Veranstaltung an der Flensburger Förde"
        title={event.title}
        intro={
          event.description ||
          `Alle verfügbaren Informationen zu ${event.title} an der Flensburger Förde.`
        }
        bullets={[
          `Datum: ${dateLabel}`,
          `Uhrzeit: ${timeLabel}`,
          event.location ? `Ort: ${event.location}` : "Ort: siehe Originalquelle",
          event.organizer
            ? `Veranstalter: ${event.organizer}`
            : "Veranstalter: siehe Originalquelle",
        ]}
      ...
      >
        <section className="seo-card">
          ...
          <p><strong>Uhrzeit:</strong> {timeLabel}</p>
          {event.location ? <p><strong>Ort:</strong> {event.location}</p> : null}
          {event.organizer ? (
            <p><strong>Veranstalter:</strong> {event.organizer}</p>
          ) : null}
        ...
        <section className="seo-card">
          ...
          <p>
            Veranstaltungen können kurzfristig geändert oder abgesagt werden.
            Prüfe deshalb vor dem Besuch noch einmal die verlinkte Originalquelle.
          </p>
        </section>
      </SeoLanding>
    </>
